...
import { byDate, EmptyDay, EmptyInterval } from './data.helpers';
...
export class DataService {
  ...
  private getIntervalCases(start: Date, stop: Date): casesInInterval {
    ...
    if (interval.length === 0) {
      ...
      return interval.reduce((interval, { cases }) => {
        for (let i = 0; i < interval.cases.length; i++) {
          interval.cases[i].cases += cases[i].cases;
        }
        return interval;
